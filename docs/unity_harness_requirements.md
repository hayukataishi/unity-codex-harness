...
>
> このファイルは、ハーネスを導入したUnityプロジェクトへ適用する
...
- `必須標準`要件は導入時点で継承され、通常のゲーム判断では無効化しない。
- `決定必須`要件は、選択肢、推奨理由、トレードオフをユーザーと確認し、関連実装へ進む前にゲーム設計書で選択を確定する。
- `条件付き推奨`要件は適用条件をユーザーと確認し、条件を満たす場合に採用する。満たさない場合も`対象外`の理由を記録する。
- 標準と異なる運用は、ゲーム設計書で`例外承認`、理由、影響、代替策、承認者を記録する。
- `未決定`、未承認の例外、標準要件との矛盾がある場合、Codexは依存する実装を開始または受け入れしない。

文書内の章番号は説明を読む順番であり、ゲーム設計書との対応番号ではない。標準要件と個別要件の対応は、両文書で共通の`HREQ-*` IDを使う。
...
asmdefは[HREQ-ARCH-001](#architecture-profile-gate)で選択したプロファイルに合わせる。新規プロジェクトは将来の規模を先取りせず、現在の制約を満たす最小の構成から開始する。既存プロジェクトは現在のAssembly境界を調査し、プロファイルへ合わせるためだけの一括再編を行わない。
...
### HREQ-CAMERA-001: Unity 6のCinemachine例は3.x APIを基準とする
...
|---|---|---|---|---|
| `HREQ-CAMERA-001-AC01` | `有効` | `AUTO:STATIC` | Unity 6向けカメラ例が`CinemachineCamera`とCinemachine 3.xのPosition / Rotation Control Componentを使用している | リポジトリ文書検査 |
| `HREQ-CAMERA-001-AC02` | `有効` | `AUTO:STATIC` | Packageバージョン検出、`Unity.Cinemachine`名前空間、Tracking Target、Cinemachine Channelが規定されている | リポジトリ文書検査 |
| `HREQ-CAMERA-001-AC03` | `有効` | `AUTO:STATIC` | Cinemachine 2.x既存案件を互換対象とし、3.x移行にUpgraderと参照検査が必要だと規定されている | リポジトリ文書検査 |
...
### HREQ-ARCH-001: プロジェクト規模に合うアーキテクチャプロファイルを選択する
...
| AC ID | 状態 | 検証種別 | 合格条件 | 検証方法 |
|---|---|---|---|---|
| `HREQ-ARCH-001-AC01` | `有効` | `AUTO:STATIC` | `Small`、`Standard`、`Large`の適用状況、構造、非必須事項と選択記録が定義されている | リポジトリ文書検査 |
| `HREQ-ARCH-001-AC02` | `有効` | `AUTO:STATIC` | asmdef、Layer、DI、Manager、イベント方式がProfile別の選択事項であり、Service Locatorを規模別の推奨方式にしていない | リポジトリ文書検査 |
| `HREQ-ARCH-001-AC03` | `有効` | `AUTO:STATIC` | Profile間の観測可能な再評価条件、人間承認、影響調査、段階移行が規定されている | リポジトリ文書検査 |
...
### HREQ-SAVE-001: 進行データを破損とVersion差から復旧できる
...
| AC ID | 状態 | 検証種別 | 合格条件 | 検証方法 |
|---|---|---|---|---|
| `HREQ-SAVE-001-AC01` | `有効` | `AUTO:STATIC` | Atomic write、Backup、Integrity、復旧順、書込み直列化と失敗時UXの決定欄が存在する | リポジトリ文書検査 |
| `HREQ-SAVE-001-AC02` | `有効` | `AUTO:STATIC` | Schema migration、未来Version、downgrade、最古対応Version、Migration前Rollbackが規定されている | リポジトリ文書検査 |
| `HREQ-SAVE-001-AC03` | `有効` | `AUTO:STATIC` | Cloud競合、Platform制約、暗号・鍵管理、PrivacyとPlayerPrefsの用途制限が規定されている | リポジトリ文書検査 |
| `HREQ-SAVE-001-AC04` | `有効` | `AUTO:STATIC` | 旧Version fixture、書込み中断、破損、Backup、未来Version、失敗処理、Cloud競合のテスト行列が存在する | リポジトリ文書検査 |
...
- [ ] **バージョン管理**（[HREQ-REPO-001](#project-002)のGit / LFS / Unity Merge / Ownership選択）
- [ ] **ビルド対象プラットフォームと品質設定**
...
### HREQ-REPO-001: リポジトリ特性に合うGit・大容量アセット運用を選択する
...
| AC ID | 状態 | 検証種別 | 合格条件 | 検証方法 |
|---|---|---|---|---|
| `HREQ-REPO-001-AC01` | `有効` | `AUTO:STATIC` | Branch strategyが選択式で、Default branch、CI、Review、Release / hotfix、再評価条件を記録する | リポジトリ文書検査 |
| `HREQ-REPO-001-AC02` | `有効` | `AUTO:STATIC` | LFSが拡張子一律ではなくsize・変更頻度・Merge可否・Quotaで判断され、`.meta`、Generated file、履歴rewriteの規則がある | リポジトリ文書検査 |
| `HREQ-REPO-001-AC03` | `有効` | `AUTO:STATIC` | Visible Meta Files、Force Text、UnityYAMLMerge、`.gitattributes`、既存Project移行規則が定義されている | リポジトリ文書検査 |
| `HREQ-REPO-001-AC04` | `有効` | `AUTO:STATIC` | Scene、Prefab、ProjectSettings、Binary AssetのOwnership、Lock、分割、競合後検証が定義されている | リポジトリ文書検査 |
...
### HREQ-BUILD-001: Unity 6では保存済みBuild Profileをビルド構成の正とする
...
| AC ID | 状態 | 検証種別 | 合格条件 | 検証方法 |
|---|---|---|---|---|
| `HREQ-BUILD-001-AC01` | `有効` | `AUTO:STATIC` | Unity 6のScene・Build説明が保存済みBuild ProfileとScene Listを正としている | リポジトリ文書検査 |
| `HREQ-BUILD-001-AC02` | `有効` | `AUTO:STATIC` | Development / QA / Releaseの用途、Defines、Debug設定、Clean Build条件、保存場所、命名が定義されている | リポジトリ文書検査 |
| `HREQ-BUILD-001-AC03` | `有効` | `AUTO:STATIC` | CI手順が`-activeBuildProfile`でProfileアセットを明示し、Editorの前回状態へ依存しない | リポジトリ文書検査 |
...
| 状態 | 必須記録 | 実装可否 |
|---|---|---|
| `未決定` | 初期対話前または判断材料不足。Blocking未決事項、決定責任者、決定期限またはマイルストーン | 関連する依存導入・実装を開始しない |
...
### HREQ-CROSS-001: 横断機能の採否を実装前に決定する

**仕様**

- すべての横断領域を初期状態の`未決定`から`採用`、`不採用`、`保留`のいずれかへ分類し、`未決定`または空欄を残したまま依存実装へ進まない。
...
| AC ID | 状態 | 検証種別 | 合格条件 | 検証方法 |
|---|---|---|---|---|
| `HREQ-CROSS-001-AC01` | `有効` | `AUTO:STATIC` | 14領域の採否マトリクスと`採用` / `不採用` / `保留`の記録規則が設計書に存在する | リポジトリ文書検査 |
| `HREQ-CROSS-001-AC02` | `有効` | `AUTO:STATIC` | `採用`領域に設計ID・AC、依存、データ考慮を要求し、`不採用`・`保留`に理由と再評価条件を要求する | リポジトリ文書検査 |
| `HREQ-CROSS-001-AC03` | `有効` | `AUTO:STATIC` | 高影響領域の人間承認、保留中の実装停止、マイルストーンと条件変更時の再評価が運用規則に含まれる | リポジトリ文書検査 |
