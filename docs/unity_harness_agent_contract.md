<!-- UNITY_CODEX_HARNESS_AGENT_CONTRACT: HARNESS-MANAGED -->

# Unity Codex Harness Agent Contract

This harness-managed document contains the mandatory Codex working contract.
The project-owned root `AGENTS.md` must contain:
...
Projects may keep any additional repository instructions in `AGENTS.md`.

## Required context

Before Unity design, implementation, validation, asset integration, gameplay
review, or reporting, read:
...
- Before Unity work in an installed game project, run
...
- Before claiming a Concept, Prototype, Vertical Slice, Alpha, Beta, or
...
- Resolve the Unity project root by verifying `Assets/`, `Packages/`, and
  ...
  absolute path.
- Update or obtain approval for affected design items and acceptance criteria
  before changing approved behavior.
- Prefer Unity MCP or Unity Editor APIs for scenes, prefabs, assets, import
  settings, tags, layers, and serialized references.
- Preserve `.meta` files and GUIDs. Do not directly edit Unity YAML unless no
  safer supported operation exists.
- Keep changes small, inspect the diff, and avoid unrelated refactors or asset
  moves.
- Store validation evidence under `Artifacts/ValidationRuns/<RunId>/` and keep
  `Artifacts/` out of Git.
- Never claim acceptance while an active acceptance criterion is `FAIL`,
  `BLOCKED`, or `NOT RUN`.
- Leave subjective judgments such as fun, feel, clarity, and acceptable
  difficulty to a human reviewer.
...
5. Use `$validate-unity-change` after code, asset, scene, prefab, or settings
   changes.
