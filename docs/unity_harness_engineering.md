...
3. 10の対話Phaseを一問ずつ進め、対象マイルストーンの必須深度だけを要求し、
   推奨前に選択肢とトレードオフを説明する。
4. 各Phaseの結果をゲーム設計書の対話進捗、関連HREQ、詳細設計、
   未決事項、承認履歴へ反映する。
5. 質問、選択肢、回答要約、反映案、確認状態を非規範な対話証跡へ保存し、
   確認待ちで中断しても再開可能にする。
6. multi-agent機能がある場合は、主Agentと独立した読み取り専用Subagentが
   各Phase終了時に設計書と対話証跡を読み、漏れ、矛盾、誘導、
   未説明トレードオフを監査する。
7. 主Agentだけがユーザーへ質問し、設計書を更新する。Subagentは承認や
   主観判断を代行しない。
8. 最終監査、HREQ契約検査、人間の明示承認後に対象マイルストーンの
   初期設計を完了する。
