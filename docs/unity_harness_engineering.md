...
- `不採用`領域は理由と再評価条件、`保留`領域は理由・責任者・期限またはマイルストーンを記録する。
- `保留`の領域に依存するPackage追加、SDK設定、通信、データ収集、課金・広告・UGC導線の実装を開始しない。
- Package、SDK、Service、対象地域、年齢区分、ビジネスモデル、プラットフォーム、収集データが変わる場合は、実装前に関連行を再評価する。
- Privacy、Consent、Security、Account、Multiplayer、Analytics、IAP / Ads、UGC、Moderationは、技術実装だけで法令・Store規約・運用責任を確定した扱いにしない。人間の承認と必要な専門レビューを記録する。
- Codexは法務判断を代替せず、未決事項を`保留`または`要確認`として報告する。
...
1. 新規ゲームまたは未完成設計では`$bootstrap-game-design`を使用する。
2. 対象マイルストーン、意思決定者、既存資料、今回決める深さを確認する。
3. 10の対話Phaseを一問ずつ進め、対象マイルストーンの必須深度だけを要求し、
   推奨前に選択肢とトレードオフを説明する。
4. 各質問を`標準推奨`、`ゲーム個別`、`両方`へ分類する。標準推奨では
   関連HREQの採否・方式・例外を、ゲーム個別ではプレイヤー体験と個別仕様を
   決め、`両方`では二つの記録を同期する。
5. 各Phaseの結果をゲーム設計書の対話進捗、関連HREQ、詳細設計、
   未決事項、承認履歴へ反映する。
6. 質問区分、関連HREQ、質問、選択肢、回答要約、反映案、確認状態を
   非規範な対話証跡へ保存し、
   確認待ちで中断しても再開可能にする。
7. ユーザーがSubagent利用を明示し、multi-agent機能がある場合は、
   `.codex/agents/game-design-auditor.toml`で定義した読み取り専用Custom Agentが
   各Phase終了時に設計書と対話証跡を読み、漏れ、矛盾、誘導、
   未説明トレードオフを監査する。
8. 主Agentだけがユーザーへ質問し、設計書を更新する。Subagentは承認や
   主観判断を代行しない。
9. 最終監査とHREQ契約検査後に`validate_design_readiness.py`を対象
   マイルストーン指定で実行する。必須Phase、設計欄、HREQ、未決事項、
   横断機能、設計ID・AC、監査、承認のいずれかが不足すれば完了しない。
10. 完成度検査`PASS`と人間の明示承認後に対象マイルストーンの初期設計を
    完了する。

### 5.1 設計書メンテナンス

初期設計後の差分変更には`$maintain-game-design`を使用する。
